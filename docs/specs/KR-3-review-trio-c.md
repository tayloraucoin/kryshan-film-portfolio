# KR-3 — Review trio C: kit C, layout C ("The Index"), mock home C
...
Behind the gate, `/review/kits/kryshan-c`, `/review/layouts/kryshan-c` and `/review/mocks/home-c` render kit C as a live style page, layout C's markdown verbatim inside kit C's scope, and a Home-only mock built to `kryshan-06-layout-C.md` §4 and §6 with his real titles, posters and copy. `/review` lists the trio. Reuses KR-1's content; exposes the three role-tag colours as `--tag-directing`, `--tag-camera`, `--tag-editing`, used only on tags and the page's active labels. This slice builds Home only: no Work, About, Teaching, Credits or Contact pages.

## Why / intent

- **`docs/client/prompts/kryshan-07-cc-prompt-C.md`** — the deliverable-by-deliverable brief. It is the spec for this ticket; this file carries the acceptance criteria and the records. Where the prompt and the repo's types or docs disagree, the repo wins and the divergence is one `DEVIATIONS.md` line.
- **`docs/client/branding/kryshan-05-tokens.json → C`** — the only source of colour values. No hex outside `review/kits/`.
- **`docs/client/kryshan-06-layout-C.md`** — the layout; copied verbatim, never rewritten.
...
`review/kits/kryshan-c.ts` · `review/layouts/kryshan-c.md` (+ registry entry) · `review/mocks/home-c.tsx` with client leaves in `review/mocks/_components/` · index wiring in `review/kits/index.ts`, `review/layouts/index.ts`, `review/mocks/index.ts`.
...
- **KR-1** — shared content and posters. Complete in `PROGRESS.md`.
